...
export default function TrackMaps({ activeAlbum, onAlbumChange }: {
    ...
    return <section id="tracks" className="circuits section-wrap"><div className="section-heading"><div><p className="eyebrow">01 / THE CIRCUITS</p><h2>Pick your playground.</h2></div><button className="text-button" aria-pressed={activeAlbum === 'all'} onClick={() => onAlbumChange('all')}>All circuits <span aria-hidden="true"><ArrowUpRight /></span></button></div><div className="circuit-grid" aria-label="Filter photos by circuit">{TRACKS.map((track, i) => <button key={track.album} className={`circuit ${activeAlbum === track.album ? 'selected' : ''}`} aria-pressed={activeAlbum === track.album} aria-controls="gallery" onClick={() => onAlbumChange(track.album)}><div className="circuit-top"><span className="eyebrow">0{i + 1}</span><span className="circuit-arrow" aria-hidden="true"><ArrowUpRight /></span></div><img src={track.svg} alt={`${track.name} circuit layout`}/><span className="circuit-name">{track.name}</span><span className="circuit-details">{track.detail} <span>·</span> {track.turns} turns</span><span className="circuit-region">{track.region}</span></button>)}</div></section>;
}
